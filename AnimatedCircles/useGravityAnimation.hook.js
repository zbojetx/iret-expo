...
import { circleDiameter, circleHeight } from "./Circle.component";
...
const useDraw = circles => {
    const nativeCode = useMemo(() => {
        ...
        for (let i = 0; i < circles.length; i++) {
            ...
            nativeCode.push(set(circle.x, add(circle.x, multiply(circle.x, -0.005))));
            nativeCode.push(set(circle.y, add(circle.y, multiply(circle.y, -0.005))));
        }
        ...
        for (let i = 0; i < circles.length; i++) {
            for (let j = i; j < circles.length; j++) {
                ...
                const areOverlapping = lessThan(distanceBetweenCenters, circleHeight);

                const overlapDistance = sub(circleHeight, distanceBetweenCenters);
                const percentOverlap = divide(overlapDistance, circleHeight);
